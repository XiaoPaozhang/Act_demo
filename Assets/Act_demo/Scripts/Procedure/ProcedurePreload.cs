...
namespace Act_demo
{
  public class ProcedurePreload : ProcedureBase
  {
    ...
    protected override void OnUpdate(ProcedureOwner procedureOwner, float elapseSeconds, float realElapseSeconds)
    {
      ...
      if (m_LoadedFlag.Any(v => v.Value == false))
        return;

      // 注入luban
      Tables table = new Tables((file) =>
      {
      ...
      GameEntry.LubanComponent.SetTables(table);

      //清空临时存放luban加载的资源
      m_LubanTextAssets.Clear();

      //切换到默认场景
      procedureOwner.SetData<VarInt32>("NextSceneId", (int)ProcedureChangeScene.defaultSceneType);
    ...
    private void PreloadResources()
    {
      ...
      foreach (string lubanTableName in LubanTableNames)
      {
    ...
    private void LoadLubanTable(string lubanTableName)
    {
      ...
      // }
      lubanTableAssetName = AssetUtility.GetLubanTableAsset(lubanTableName);
      ...
      {
        m_LoadedFlag[lubanTableAssetName] = true;
